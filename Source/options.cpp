...
void DisableEnhancedOptionsIfShareware()
{
	auto &options = GetOptions();
	auto &gameplay = options.Gameplay;

	const bool isShareware = gbIsSpawn;
	if (!isShareware)
		return;

	gameplay.disableSearch.SetValue(false);
	gameplay.disableSearch.flags |= OptionEntryFlags::Invisible;

	gameplay.sharedXP.SetValue(false);
	gameplay.sharedXP.flags |= OptionEntryFlags::Invisible;
...
void LoadOptions()
{
	...
	DisableEnhancedOptionsIfShareware();
}
