...
EnhancedOptions::EnhancedOptions()
    ...
    , disableSearch("Disable Search Spell", OptionEntryFlags::CantChangeInMultiPlayer | OptionEntryFlags::OnlyHellfire, N_("Disable Search Spell"), N_("Search spell will not appear on Scrolls, Books, or Staves generated by monsters you kill or objects and vendors you interact with.  Set Monk class skill to Infravision."), false)
    , AutoMapShowItems("Show Items on Map", OptionEntryFlags::None, N_("Show Items on Map"), N_("Blue squares will now be drawn on the mini map identifying the location of an item when the map is enabled."), false)
	, enableMonkDiablo("Enable Monk in Diablo", OptionEntryFlags::CantChangeInMultiPlayer | OptionEntryFlags::NeedHellfireMpq, N_("Enable Monk in Diablo"), N_("Allows selection of the Monk class when playing Diablo mode with hellfire.mpq loaded (hfmonk.mpq and hfvoice.mpq also required)."), false)
