...
| `OnGolemMissileCanHitGolem(golem, target) -> bool\|nil` (query) | Fires in `CheckMissileCol`'s monster-hit faction gate when **both** the missile's source and the monster in its path are **MFLAG_GOLEM** player-minions — a pairing vanilla **never resolves** (the gate requires opposing factions or a berserk flag, so a minion-vs-minion missile always passes through), meaning the hook can only ever **widen** resolution, never narrow vanilla. Return **true** to admit the hit — it then routes through `OnMonsterMissileHit` like any minion-involved hit (single-resolver authority, engine damage broadcast); default **false** = the vanilla pass-through. Completes the pet-vs-pet combat surface: `OnGolemCanTargetGolem` permits the fight at the targeting layer, this makes ranged damage deliverable at the resolution layer (melee never needed a gate). The mod's policy is the targeting rule verbatim: **mutually-hostile owners** open the pair; same-owner/peaceful pairs (vanilla Golem included, either end) keep the pass-through, so a stray boom crossing a friendly pet's tile never resolves. | `Source/missiles.cpp` `CheckMissileCol` |
...
| `OnGuardianCanTargetGolem(player, golem) -> bool\|nil` (query) | Fires from the **Guardian target search** (`GuardianTryFireAt`) when it evaluates a **MFLAG_GOLEM** monster — vanilla always skips player-minions (its own `isPlayerMinion` return), so the hook can only ever **widen** the search, never narrow vanilla. Return **true** to let the caster's Guardian fire at this minion; default **false** = the vanilla exclusion. The firebolt's damage then resolves normally through `CheckMissileCol` → `OnPlayerMissileCanHitGolem` → `MonsterMHit`. The mod's policy is `OnApocalypseCanTargetGolem`'s verbatim: a **hostile** caster's Guardian may fire at another player's tamed ally; own/peaceful pets keep the vanilla protection unconditionally (a Guardian never fires at players, so peace-time pets stay untouchable independent of Friendly Fire); a vanilla Golem keeps the vanilla exclusion. | `Source/missiles.cpp` `GuardianTryFireAt` |
...
- `items.findFloorItemBySeed(seed) -> Item|nil` — the live floor Item on the current level whose seed matches, or nil; live reference (name/quality mutations show immediately, e.g. restamping a floor scroll's presentation when its identity blob arrives after the item wire). Source: `Source/lua/modules/items.cpp`
...
- `player.weaponGraphic` (readonly integer) — the `PlayerWeaponGraphic` id of the equipped weapon combo (Unarmed=0, UnarmedShield=1, Sword=2, SwordShield=3, Bow=4, Axe=5, Mace=6, MaceShield=7, Staff=8), i.e. the low nibble of `_pgfxnum` — the weapon sheet the player's animations play. Synced equipment state, safe for deterministic per-client hooks (used by the Monk-archetype staff/unarmed attack-frame override in `OnGetAnimationSkipFrames`).
...
- `statPoints` (integer, **read/write**) — unspent stat points (`_pStatPts`). Writable because the hero file stores it as a single BYTE: any mod value >255 wraps on save→load, so the true value must be persisted mod-side (Hunter: `OnSavePlayerData` trailing field + `restoreStatPoints()` at GameStart). Setter floors at 0
...
- `monster:encodedEnemy() -> integer|nil` — the monster's current enemy in the engine's **wire encoding** (`encode_enemy`: monster target = the monster id, player target = playerId + `GetMaxMonsters()`), or nil when it has no enemy (`MFLAG_NO_ENEMY`). Hunter reads it owner-side for the `EN` pet-target-convergence broadcast (see `net_sync.md`)
- `monster:setEncodedEnemy(enemyId: integer|nil)` — set the monster's enemy from the wire encoding — the **same application the engine's own monster sync performs** (`decode_enemy`: enemy + `enemyPosition` + the `MFLAG_TARGETS_MONSTER` dispatch flag), validated like an incoming sync record (`IsEnemyValid`; invalid/dead targets ignored) and clearing `MFLAG_NO_ENEMY` on success. `nil` clears the enemy (sets `MFLAG_NO_ENEMY`). Hunter applies it in the `EN` receiver so peers converge on the owner's latched pet target
