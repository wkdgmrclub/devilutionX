...
own copies of the same unique. Evaluated on the caster over its own `deployedAllies` **plus its in-flight
`pendingDeploys`** (the DR round-trip window counts as deployed, else rapid casting over-deploys before the
SP echoes land) (`OnCanCastScroll` + the `OnSpellActionFrame` refund net), and re-checked authoritatively by
the level owner on the `DR` path over the requester's `remoteAllies` records.
